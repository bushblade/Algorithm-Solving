  ...
  const inventorsBorn = inventors.filter(x => x.year >= 1500 && x.year < 1600)
  ...
  const years = inventors.reduce((sum, x) => sum += (x.passed - x.year), 0)
  console.log(years)
  ...
  const ageSort = inventors.sort((a, b) => getAge(a) - getAge(b)).map(x => getAge(x))
  console.log(ageSort)
  ...
  const sortPeople = people.sort((a, b) => lastName(a) > lastName(b) ? 1 : -1)
